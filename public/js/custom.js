...
jQuery(document).ready(function($) {
    ...
    function tabsAnimation(tabs) {
        ...
        $this
            .find('.tabs-content')
            .css('height', $this.find('.content.active').height());

        $link.on('click', function() {
            $this
                .find('.tabs-content')
                .css('height', $this.find($(this).attr('href')).height());
        });
    }
            ...
            $items.each(function(i) {
                $(this)
                    ...
                    .on('click.accordion', function() {
                        if (
                            !options.toggleable &&
                            $items.eq(0).hasClass(options.active_class)
                        ) {
                            return;
                        }
    ...
    $(document).ajaxComplete(function() {
        $('#project-gallery').owlCarousel({
            ...
            navigation: true,
            navigationText: [
                "<i class='icon tyger-chevron-left-1 '></i>",
                "<i class='icon tyger-chevron-right-1 '></i>"
            ]
        });
    });
